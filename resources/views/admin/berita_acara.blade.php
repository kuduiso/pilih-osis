...
<div class="mt-4">
    ...
    <form action="{{ action('AdminController@proses_berita_acara') }}" method="POST" class="mx-auto flex flex-col w-full md:w-1/2">
        ...
        <input type="text" class="form-input" name="nama_ketua" id="nama_ketua" value="{{ old('nama_ketua') }}" required>
        @error('nama_ketua')
        <span class="text-red-500 text-sm">*{{ $message }}</span>
        @enderror
        <label for="tempat" class="mt-2">Tempat Pemilihan:</label>
        <input type="text" class="form-input" name="tempat" id="tempat" value="{{ old('tempat') }}" required>
        @error('tempat')
        <span class="text-red-500 text-sm">*{{ $message }}</span>
        @enderror
        <label for="tanggal" class="mt-2">Tanggal Pemilihan:</label>
        <input type="date" class="form-input" name="tanggal" id="tanggal" value="{{ old('tanggal') }}" required>
        @error('tanggal')
        <span class="text-red-500 text-sm">*{{ $message }}</span>
        @enderror
